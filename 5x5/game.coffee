...
printGrid = ->
  ...
  rowSeperator = ('-' for i in [1..GRID_SIZE * 4]).join('')
  console.log '\n' + rowStrings.join("\n#{rowSeperator}\n") + '\n'
...
promptForTile1 = ->
  ...
  inputCallback = (input) ->
    if coordinates = strToCoordinates input
      promptForTile2 coordinates.x, coordinates.y

promptForTile2 = (x1, y1) ->
  console.log "Please enter coordinates for the second tile."
  inputCallback = (input) ->
    if {x: x2, y: y2} = strToCoordinates input
